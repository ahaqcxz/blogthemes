...
$summaryLength = !empty($isFeaturePost) ? 220 : 132;
...
<article class="<?= h($cardClass) ?>">
    ...
    <div class="post-card-main">
        <div class="post-meta"><a href="/sort/<?= (int)$post['category_id'] ?>.html">#<?= h($categoryName) ?></a><span><?= h(display_date($post['created_at'], 'Y-m-d')) ?></span><?php if (post_is_protected($post)): ?><span>locked</span><?php endif; ?></div>
        <h2><a href="/read/<?= (int)$post['id'] ?>.html"><?= h($post['title']) ?></a></h2>
        <p><?= h($summary) ?></p>
        <div class="post-card-foot"><span>@<?= h($post['author_name'] ?: 'admin') ?></span><span><?= (int)$post['views'] ?> reads</span><a href="/read/<?= (int)$post['id'] ?>.html">open</a></div>
    </div>
</article>
