...
<header class="holo-header">
    <a class="holo-brand" href="/">
        <span><?= h(utf8_limit($logoText, 1)) ?></span>
        <strong><?= h($logoText) ?></strong>
    </a>
    <button class="nav-toggle" type="button" onclick="document.getElementById('holoNav').classList.toggle('is-open')" aria-label="打开导航" aria-controls="holoNav">
        <span></span><span></span><span></span>
    </button>
    <nav class="holo-nav" id="holoNav" aria-label="主导航">
        ...
        <form class="nav-search" onsubmit="siteSearch('holoMobileKeyword');return false;">
            <label class="sr-only" for="holoMobileKeyword">搜索文章</label>
            <input id="holoMobileKeyword" placeholder="搜索">
            <button type="submit">GO</button>
        </form>
    </nav>
</header>
<main id="mainContent" class="holo-main">
