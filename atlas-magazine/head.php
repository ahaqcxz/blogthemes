...
<header class="atlas-header">
    <div class="atlas-wrap atlas-topbar">
        <a class="atlas-brand" href="/">
            <span><?= h(utf8_limit($logoText, 1)) ?></span>
            <strong><?= h($logoText) ?></strong>
        </a>
        <button class="nav-toggle" type="button" onclick="document.getElementById('atlasNav').classList.toggle('is-open')" aria-label="打开导航" aria-controls="atlasNav">
            <span></span><span></span><span></span>
        </button>
        <nav class="atlas-nav" id="atlasNav" aria-label="主导航">
            <a class="<?= $path === '/' ? 'is-active' : '' ?>" href="/">首页</a>
            <?php foreach (array_slice(Blog::categories(), 0, 5) as $navCategory): ?>
            ...
            <form class="nav-search" onsubmit="siteSearch('atlasNavKeyword');return false;">
                <label class="sr-only" for="atlasNavKeyword">搜索文章</label>
                <input id="atlasNavKeyword" placeholder="搜索">
                <button type="submit">搜索</button>
            </form>
        </nav>
    </div>
</header>
<main id="mainContent" class="atlas-main atlas-wrap">
