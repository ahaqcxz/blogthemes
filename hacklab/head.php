...
<header class="site-header">
    <div class="site-wrap topbar">
        <a class="terminal-brand" href="/" aria-label="<?= h($logoText) ?>">
            <span class="prompt">root@blog</span><strong><?= h($logoText) ?></strong><span class="cursor"></span>
        </a>
        <button class="nav-toggle" type="button" onclick="document.getElementById('siteNav').classList.toggle('is-open')" aria-label="打开导航" aria-controls="siteNav"><span></span><span></span><span></span></button>
        <nav class="site-nav" id="siteNav" aria-label="主导航">
            ...
            <?php endforeach; ?>
            <a class="<?= !empty($linksPage) ? 'is-active' : '' ?>" href="/links.html">links</a>
            <a class="<?= $messageBoard ? 'is-active' : '' ?>" href="/bbs.html">bbs</a>
            <form class="mobile-search" onsubmit="siteSearch('hackMobileKeyword');return false;">
                <label class="sr-only" for="hackMobileKeyword">搜索文章</label>
                <input id="hackMobileKeyword" placeholder="grep -r keyword">
                <button type="submit">exec</button>
            </form>
        </nav>
    </div>
    <div class="site-wrap statusline" aria-label="站点状态">
        <span>site: <?= h($site['site_name'] ?? $logoText) ?></span>
