<aside class="river-sidebar" aria-label="内容索引">
    <section class="side-panel side-search-panel">
        <p class="side-label">Search</p>
        <form class="side-search" onsubmit="siteSearch('sideKeyword');return false;">
            <label class="sr-only" for="sideKeyword">搜索文章</label>
            <input id="sideKeyword" placeholder="输入关键词">
    ...
    <section class="side-panel">
        ...
        <div class="category-index">
            <?php foreach (Blog::categories() as $sideCategory): ?>
                <a href="/sort/<?= (int)$sideCategory['id'] ?>.html">
                    <span><?= h($sideCategory['name']) ?></span>
                    <em><?= count(Blog::postsByCategory((int)$sideCategory['id'])) ?></em>
                </a>
            <?php endforeach; ?>
        </div>
    </section>
    <section class="side-panel">
        <h3>热门文章</h3>
        <ol class="rank-list">
            <?php foreach (Blog::hotPosts() as $hotIndex => $hot): ?>
                <li><a href="/read/<?= (int)$hot['id'] ?>.html"><span><?= (int)$hotIndex + 1 ?></span><?= h($hot['title']) ?></a></li>
            <?php endforeach; ?>
        </ol>
    </section>
    <section class="side-panel">
